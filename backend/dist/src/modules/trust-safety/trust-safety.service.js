...
import { GuideStatus, ModerationActionType, ReportStatus, ReportTargetType, UserModerationStatus, } from '../../generated/prisma/client.js';
...
let TrustSafetyService = class TrustSafetyService {
    ...
    async assertInteractionAllowed(firstUserId, secondUserId) {
        ...
        if (blocked)
            throw new ForbiddenException({ code: 'USER_INTERACTION_BLOCKED', message: 'This interaction is not available' });
    }
    ...
    async dismissReport(adminId, reportId, reason) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!changed.count)
                throw new ConflictException('Report is already closed or does not exist');
    ...
    async moderate(adminId, reportId, dto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            switch (dto.action) {
                ...
                case ModerationActionType.GUIDE_VERIFICATION_REVOKE: {
                    if (report.targetType !== ReportTargetType.GUIDE)
                        throw new BadRequestException('This action requires a guide report');
                ...
                case ModerationActionType.TEMPORARY_SUSPENSION:
                    if (report.targetType !== ReportTargetType.USER || !dto.durationHours)
                        throw new BadRequestException('Temporary suspension requires a user report and duration');
                ...
                case ModerationActionType.PERMANENT_SUSPENSION:
                    if (report.targetType !== ReportTargetType.USER)
                        throw new BadRequestException('Permanent suspension requires a user report');
