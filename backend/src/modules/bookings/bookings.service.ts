...
export class BookingsService {
  ...
  async updateStatus(userId: string, bookingId: string, action: BookingAction) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (booking.listingId && (
        transition.to === BookingStatus.DECLINED ||
        transition.to === BookingStatus.CANCELLED_BY_TRAVELER ||
  ...
  private resolveTransition(booking: BookingWithOwner, action: BookingAction, isTraveler: boolean, isProvider: boolean) {
    if (!isTraveler && !isProvider) throw new ForbiddenException();
    ...
    switch (action) {
      case BookingAction.ACCEPT:
        if (!isProvider || booking.status !== BookingStatus.PENDING) return invalid();
      ...
      case BookingAction.START:
        if (!isProvider || booking.status !== BookingStatus.CONFIRMED) return invalid();
        return { to: BookingStatus.IN_PROGRESS, message: 'Booking started' };
      case BookingAction.COMPLETE:
        if (!isProvider || booking.status !== BookingStatus.IN_PROGRESS) return invalid();
        return { to: BookingStatus.COMPLETED, message: 'Booking completed' };
      case BookingAction.DISPUTE:
        if (booking.status !== BookingStatus.IN_PROGRESS && booking.status !== BookingStatus.COMPLETED) return invalid();
        return { to: BookingStatus.DISPUTED, message: 'A booking dispute was opened' };
    }
  }

  private replay(storedHash: string, requestHash: string, responseBody: Prisma.JsonValue | null) {
  ...
  private async reserveListingInventory(
    tx: Prisma.TransactionClient,
    listingId: string,
    startsAt: Date,
    endsAt: Date,
    ...
    for (const date of this.inventoryDates(startsAt, endsAt)) {
      ...
      if (reserved.count !== 1) this.unavailable();
    }
  }

  private async releaseListingInventory(tx: Prisma.TransactionClient, listingId: string, startsAt: Date, endsAt: Date) {
    for (const date of this.inventoryDates(startsAt, endsAt)) {
