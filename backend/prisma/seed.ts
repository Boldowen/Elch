...
async function main() {
  ...
  const traveler = await prisma.user.upsert({ where: { email: 'traveler@elch.mn' }, update: { passwordHash }, create: { email: 'traveler@elch.mn', name: 'Anu Batsaikhan', passwordHash, provider: AuthProvider.EMAIL, roles: [Role.TRAVELER], avatarUrl: img.guide2, isVerified: true } });
  const admin = await prisma.user.upsert({ where: { email: 'admin@elch.mn' }, update: { passwordHash, roles: [Role.ADMIN] }, create: { email: 'admin@elch.mn', name: 'ELCH Admin', passwordHash, roles: [Role.ADMIN], isVerified: true } });
  const host = await prisma.user.upsert({ where: { email: 'guide@elch.mn' }, update: { passwordHash }, create: { email: 'guide@elch.mn', name: 'Bat-Erdene D.', passwordHash, roles: [Role.GUIDE], avatarUrl: img.guide1, isVerified: true } });
  const friend = await prisma.user.upsert({ where: { email: 'nomad@elch.mn' }, update: { passwordHash }, create: { email: 'nomad@elch.mn', name: 'Mika Tanaka', passwordHash, roles: [Role.TRAVELER], avatarUrl: img.guide1, isVerified: true } });
